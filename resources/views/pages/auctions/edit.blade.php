...
@push('topbar')
<div class="text-danger font-bold">Edit Auction</div>
@endpush
...
<div class="bg-white rounded-md w-full p-6">
    <form id="auction-form" enctype="multipart/form-data">
        <div class="flex flex-col mt-4">
            ...
            <select class="input-text" id="event-id" name="event-id">
                ...
                @foreach ($events as $key => $event)
                    <option value="{{ $event['id'] }}" {{$auction['event']['id'] == $event['id'] ? 'selected' : ''}}> {{ $event['name'] }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="flex flex-col mt-4">
            ...
            <input type="text" class="input-text" placeholder="Masukan Nama" id="name" name="name" autocomplete="off" value="{{$auction['name']}}">
        </div>

        <div class="flex flex-row space-x-4">
            <div class="flex flex-col mt-5">
                <label for="" class="text-sm tracking-wider font-bold">Start Date<span class="text-red-600 text-sm">*</span></label>
                <input type="text" class="input-text" placeholder="01 January 2022" id="start-date" name="start-date" autocomplete="off" value="{{$auction['started_at']}}">
            </div>
            <div class="flex flex-col mt-5">
                <label for="" class="text-sm tracking-wider font-bold">End Date<span class="text-red-600 text-sm">*</span></label>
                <input type="text" class="input-text" placeholder="10 January 2023" id="end-date" name="end-date" autocomplete="off" value="{{$auction['ended_at']}}">
            </div>
        </div>
        ...
        <div class="flex flex-row space-x-4">
            <div class="flex flex-col mt-5">
                ...
                <input type="text" class="input-text" placeholder="Rp. 999.999.999" id="starting-bid" name="starting-bid" autocomplete="off" value="{{$auction['start_price']}}">
            </div>
            <div class="flex flex-col mt-5">
                <label for="" class="text-sm tracking-wider font-bold">Bid Increment<span class="text-red-600 text-sm">*</span></label>
                <input type="text" class="input-text" placeholder="Rp. 999.999.999" id="bid-increment" name="bid-increment" autocomplete="off" value="{{$auction['bid_increment']}}">
            </div>
        </div>

        <div class="flex flex-col mt-5">
            <label for="" class="text-sm tracking-wider font-bold">Item Description<span class="text-red-600 text-sm">*</span></label>
            <textarea type="text" class="textarea" rows="4" placeholder="Type somethin here" id="description" name="description" autocomplete="off" value="{{$auction['description']}}">{{$auction['description']}}</textarea>
        </div>
        ...
        {{-- @php
            dd($auction);
        @endphp --}}
        <div class="mt-5 space-x-4 image-section flex flex-row">

            @if ($auction['auctionAttachments'])
                @foreach ($auction['auctionAttachments'] as $attachment)
                    <div class="relative w-24 h-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 stroke-current text-danger absolute right-0 -mr-3 -mt-3 cursor-pointer remove-image" fill="none" viewBox="0 0 24 24" stroke="currentColor" data-target="{{$attachment['id']}}">
...
<script src="{{asset('js/pages/auctions/edit.js')}}" defer></script>
